...
echo  Frame extraction rate : %EXTRACT_FPS% fps
echo ==============================================================
...
:: ================================================================
:GET_VIDEO_DURATION
::  %1 = full path to video
::  Returns: DURATION_SECONDS (as integer, rounded)
:: ================================================================
setlocal
set "VIDEO=%~1"
set "DURATION_SECONDS=0"

:: Use FFmpeg to extract duration. Redirect stderr to temp file.
set "TEMP_DUR=%TEMP%\ffmpeg_duration.txt"
"%FFMPEG%" -hide_banner 2>"%TEMP_DUR%" <nul >nul

:: Search for "Duration:" line in the output
for /f "tokens=1,2,3,4 delims=: " %%A in ('findstr /R "Duration:" "%TEMP_DUR%"') do (
    if "%%A"=="Duration" (
        set "HH=%%B"
        set "MM=%%C"
        set "SS=%%D"
        ...
        :: Remove leading zeros and decimals for math
        for /f "delims=." %%X in ("!SS!") do set "SS=%%X"
        
        :: Convert to total seconds: (HH * 3600) + (MM * 60) + SS
        set /a DURATION_SECONDS=!HH!*3600 + !MM!*60 + !SS!
    )
)

del /q "%TEMP_DUR%" 2>nul

endlocal & set "DURATION_SECONDS=%DURATION_SECONDS%"
goto :eof
...
::  %2 = extract fps (e.g., 15, 0.5, 1.0)
::  Returns: EXPECTED_FRAMES (as integer, rounded)
:: ================================================================
setlocal
set "DUR=%~1"
set "FPS=%~2"

:: Simple integer multiplication (assumes EXTRACT_FPS is integer for now)
:: For decimal fps values, we'd need more complex math;
:: For now, multiply duration by fps
set /a EXPECTED_FRAMES=%DUR% * %FPS% / 1

endlocal & set "EXPECTED_FRAMES=%EXPECTED_FRAMES%"
goto :eof
...
:: ----------------------------------------------------------------
::  Get video duration and calculate expected frame count
:: ----------------------------------------------------------------
call :GET_VIDEO_DURATION "!VIDEO!"
set "DURATION=!DURATION_SECONDS!"

:: Calculate expected frames based on actual duration
:: Note: For decimal fps (0.5, 0.7), we do simple multiplication
:: Batch doesn't handle decimals well, so we'll use a workaround
for /f "tokens=1,2 delims=." %%A in ("%EXTRACT_FPS%") do (
    set "FPS_INT=%%A"
    set "FPS_DEC=%%B"
)
if "!FPS_DEC!"=="" set "FPS_DEC=0"

:: If fps has decimal part (e.g., 0.5, 0.7), multiply by 10 for precision
if "!FPS_INT!"=="0" (
    set /a EXPECTED_FRAMES=!DURATION! * !FPS_DEC! / 10
) else (
    set /a EXPECTED_FRAMES=!DURATION! * !FPS_INT!
)

echo        Duration: !DURATION! seconds
echo        Expected frames: ~!EXPECTED_FRAMES! images
...
:: ----------------------------------------------------------------
echo        [1/4] Extracting frames at !EXTRACT_FPS! fps (~!EXPECTED_FRAMES! images) ...
